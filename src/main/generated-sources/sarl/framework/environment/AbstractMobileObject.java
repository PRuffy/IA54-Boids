...
public abstract class AbstractMobileObject extends AbstractSituatedObject implements MobileObject {
  ...
  protected Vector2f computeSteeringTranslation(final Vector2f move, final TimeManager clock) {
    ...
    if ((length != 0f)) {
      ...
      Vector2f _vector2f = new Vector2f(this.linearMove);
      v = _vector2f;
    }
    float _x = v.getX();
    float _x_1 = v.getX();
    ...
    if ((length != 0f)) {
      ...
      float speed = MathUtil.clamp(_xifexpression_1, 0f, this.maxLinearSpeed);
  ...
  protected float computeSteeringRotation(final float move, final TimeManager clock) {
    float v = 0;
    if ((move != 0f)) {
      float acceleration = MathUtil.clamp(move, (-this.maxAngularAcceleration), this.maxAngularAcceleration);
      float _abs = Math.abs(acceleration);
    ...
    if ((v != 0f)) {
      float speed = MathUtil.clamp(v, (-this.maxAngularSpeed), this.maxAngularSpeed);
      float _lastStepDuration_1 = clock.getLastStepDuration();
