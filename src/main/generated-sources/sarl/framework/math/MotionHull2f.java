...
public class MotionHull2f extends Shape2f<MotionHull2f> {
  ...
  public Point2f getCenter() {
    return new Point2f((this.start.x + (this.direction.x / 2f)), (this.start.y + (this.direction.y / 2f)));
  }
  
  /**
   * Problème sur direction.clone
   */
  @Override
  @Pure
  public MotionHull2f translate(final Tuple2f<?> vector) {
    Point2f _point2f = new Point2f((this.start.x + vector.x), (this.start.y + vector.y));
    return new MotionHull2f(_point2f, this.direction, this.size);
  }
   ...
   * 
   * var p : Point2f = new Point2f(this.start.x + vector.x, this.start.y + vector.y);
   * 
   * 
   * return new MotionHull2f(p, this.direction.clone, this.size)
   * 
   * }
   */
  @Override
